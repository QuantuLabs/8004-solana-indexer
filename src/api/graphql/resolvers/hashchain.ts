    ...
    const { rows } = await ctx.pool.query<{ response_count: string; digest: string; created_at: string }>(
      `WITH ordered AS (
         SELECT
           (ROW_NUMBER() OVER (
             PARTITION BY asset
             ORDER BY block_slot ASC, tx_signature ASC, tx_index ASC NULLS LAST, event_ordinal ASC NULLS LAST, id ASC
          ...
          `WITH ordered AS (
             SELECT
               ...
               (ROW_NUMBER() OVER (
                 ...
                 ORDER BY fr.block_slot ASC, fr.tx_signature ASC, fr.tx_index ASC NULLS LAST, fr.event_ordinal ASC NULLS LAST, fr.id ASC
